...
\end{equation}
gde je $\theta = \{W_i\}^{L}_{i=1}$ skup parametara koji se obučavaju, $W_i \in \mathbb{R}^{d_i \times d_{i-1}}$ i $\{a_i\}^{L}_{i=1}$ aktivacione funkcije koje se primenjuju član po član. Napomenimo da $d_0$ predstavlja broj kanala ulaza, dok je $d_L=1$.
